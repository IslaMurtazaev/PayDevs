...
class ProjectRepo(object):
    ...
    def get_timestamp(self, project_id, type_of_payment, start_date_boundary=None, end_date_boundary=None):
        try:
            db_project = ProjectORM.objects.get(id=project_id)
            ...
            if (type_of_payment == "H_P"):
                ...
                most_recent_end_work = db_project.start_date
                for hour_payment in db_project.hourpaymentorm_set.all():
                    work_time_list = hour_payment.worktimeorm_set.filter(start_work__gte=start_date_boundary,
                                                                         end_work__lte=end_date_boundary).order_by('-end_work')

                    if (len(work_time_list) > 0):
                        most_recent_work_time = work_time_list[0]

                        if (most_recent_work_time.end_work > most_recent_end_work):
                            most_recent_end_work = most_recent_work_time.end_work


                earliest_start_work = db_project.end_date
                for hour_payment in db_project.hourpaymentorm_set.all():
                    work_time_list = hour_payment.worktimeorm_set.filter(start_work__gte=start_date_boundary,
                                                                         end_work__lte=end_date_boundary).order_by('start_work')

                    if (len(work_time_list) > 0 or not earliest_start_work):
                        earliest_work_time = work_time_list[0]

                        if (earliest_work_time.start_work < earliest_start_work):
                            earliest_start_work = earliest_work_time.start_work


                timestamp = (most_recent_end_work - earliest_start_work).days
                ...
                most_recent_work_day = db_project.start_date.date()
                for month_payment in db_project.monthpaymentorm_set.all():
                    work_days = month_payment.workdayorm_set.exclude(day__gte=timezone.now().
                                                                          replace(day=1).date()).order_by('-day')

                    if (len(work_days) > 0):
                        work_day = work_days[0]

                        if (work_day.day > most_recent_work_day):
                            most_recent_work_day = work_day.day


                earliest_work_day = db_project.end_date
                if not earliest_work_day:
                    earliest_work_day = timezone.now().date()
                ...
                for month_payment in db_project.monthpaymentorm_set.all():
                    ...
                    if (len(work_days) > 0):
                        ...
                        if (work_day.day < earliest_work_day):
                            earliest_work_day = work_day.day


                timestamp = (most_recent_work_day - earliest_work_day).days
                if (timestamp < 0): timestamp = 0
            ...
            raise NoPermissionException(message="Invalid project id")
        # except:
        #     raise InvalidEntityException(source='repositories', code='invalid timestamp',
        #                                  message="Unable to measure project timestamp")
