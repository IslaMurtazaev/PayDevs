...
describe('Worked Time', async () => {
    ...
    it('test function getAll', async () => {
        ...
        new Promise((resolve, reject) => {
            ...
            fs.readFile("./src/__mocks__/__mockData__/workTime.json", 'utf8', (err, data) => {
              if (err) reject(err)
              resolve({ data: JSON.parse(data).filter(workTime => workTime.hourPaymentId === +hourPaymnetId)})
            })
          })
        )
    ...
    it('test function create', async () => {
        ...
        new Promise((resolve, reject) => {
            fs.readFile("./src/__mocks__/__mockData__/workTime.json", 'utf8', (err, data) => {
                ...
                resolve({ data: JSON.parse(data).filter(workTime => 
                    workTime.hourPaymentId === +hourPaymentId ).shift() })
            })
          })
        )
        ...
        expect(mockAxios.post).toHaveBeenCalledWith(
            `${BASE_URL}project/${workTime.projectId}/hour_payment/${workTime.hourPaymentId}/work_time/create`,
            { 
                "id": 1,
                "start_work": '2018-08-27T12:03:41.543000Z',
                "end_work": '2018-08-27T12:03:41.543000Z',
    ...
    it('test function Update', async () => {
        ...
        new Promise((resolve, reject) => {
            ...
            fs.readFile("./src/__mocks__/__mockData__/workTime.json", 'utf8', (err, data) => {
                ...
                resolve({ data: JSON.parse(data).filter(workTime => 
                    workTime.id === +workTimeId ).shift() })
            })
          })
        )

        const data =  await workTimeService.update(projectId, workTime.hourPaymentId, workTime.id, workTime);
        expect(data).toEqual(workTime);
        expect(mockAxios.put).toHaveBeenCalledTimes(1);
        
        expect(mockAxios.put).toHaveBeenCalledWith(
            `${BASE_URL}project/${projectId}/hour_payment/${workTime.hourPaymentId}/work_time/${workTime.id}/update`,
            { 
                "id": 1,
                "start_work": '2018-08-27T12:03:41.543000Z',
                "end_work": '2018-08-27T12:03:41.543000Z',
