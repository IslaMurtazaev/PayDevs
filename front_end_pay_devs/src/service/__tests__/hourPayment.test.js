...
describe("Hour Payment", async () => {
            ...
            (err, data) => {
              ...
              resolve({
                data: JSON.parse(data).filter(
                  hourPayment => hourPayment.projectId === +projectId
                )
              });
            }
          );
            ...
            (err, data) => {
              ...
              resolve({
                data: JSON.parse(data)
                  .filter(hourPayment => hourPayment.projectId === +projectId)
                  .shift()
              });
            }
          );
  ...
  it("test function Update", async () => {
        ...
        new Promise((resolve, reject) => {
          fs.readFile(
            ...
            (err, data) => {
              ...
              resolve({
                data: JSON.parse(data)
                  .filter(hourPayment => hourPayment.id === +hourPaymentId)
                  .shift()
              });
            }
          );
    ...
    expect(mockAxios.put).toHaveBeenCalledWith(
      `${BASE_URL}project/${hourPayment.projectId}/hour_payment/${
        hourPayment.id
      }/update`,
      {
        id: 1,
