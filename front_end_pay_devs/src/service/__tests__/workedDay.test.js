...
describe("Worked Day", async () => {
    ...
    mockAxios.get.mockImplementationOnce(
      url =>
        new Promise((resolve, reject) => {
          ...
          fs.readFile(
            "./src/__mocks__/__mockData__/workedDay.json",
            "utf8",
            (err, data) => {
              if (err) reject(err);
              resolve({
                data: JSON.parse(data).filter(
                  workDay => workDay.monthPaymentId === +monthPaymnetId
                )
              });
            }
          );
  ...
  it("test function create", async () => {
            ...
            (err, data) => {
              ...
              resolve({
                data: JSON.parse(data)
                  .filter(workDay => workDay.monthPaymentId === +monthPaymnetId)
                  .shift()
              });
            }
          );
    ...
    const data = await workedDayService.create(
      projectId,
      monthPaymentId,
      workDay
    );
  ...
  it("test function Update", async () => {
        ...
        new Promise((resolve, reject) => {
          fs.readFile(
            ...
            (err, data) => {
              ...
              resolve({
                data: JSON.parse(data)
                  .filter(workDay => workDay.id === +workDayId)
                  .pop()
              });
            }
          );
    ...
    const data = await workedDayService.update(
      projectId,
      monthPaymentId,
      workDayId,
      workDay
