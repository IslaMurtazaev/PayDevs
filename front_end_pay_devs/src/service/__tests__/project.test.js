    ...
    const projects = [ 
      ...
      { 
        ...
        new Promise((resolve, reject) => {
            ...
            fs.readFile("./src/__mocks__/__mockData__/project.json", 'utf8', (err, data) => {
              if (err) reject(err)
              resolve({ data: JSON.parse(data).filter(project => project.user_id === userId) })
            })
          })
        )

        const data =  await projectService.get_all();
