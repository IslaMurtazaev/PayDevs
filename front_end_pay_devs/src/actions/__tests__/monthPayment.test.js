...
describe('actions month paymnet', () => {
    it('should getAll an action to add a todo', async () => {
        ...
        const expectedAction  = {
            type: monthPaymentActionTypes.ADD_ALL, monthPayments: ["monthPayment"]
          }
    ...
    it('should getAll an action to add a todo', async () => {
        ...
        const expectedAction  = {
            type: monthPaymentActionTypes.CREATE, monthPayment
          }
          monthPaymentService.create.mockImplementationOnce((projectId, monthPayment) =>{
              return Promise.resolve(
                monthPayment
            )
          })
        ...
        return store.dispatch(monthPaymentActions.create(monthPayment.projectId, monthPayment)).then(() => {
            const action = store.getActions()
            expect(action.length).toBe(1)
            expect(action).toContainEqual(expectedAction)
    ...
    it('should getAll an action to add a todo', async () => {
        const monthPayment = { id: 1, projectId: 1, rate: 500 }
        const expectedAction  = {
            type: monthPaymentActionTypes.REMOVE, monthPaymentId: monthPayment.id
          }
        ...
        return store.dispatch(monthPaymentActions.remove(monthPayment.id)).then(() => {
            const action = store.getActions()
            expect(action.length).toBe(1)
            expect(action).toContainEqual(expectedAction)
