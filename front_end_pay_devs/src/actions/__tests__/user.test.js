...
describe('actions Worked Day', () => {
    it('should authentication an action to add a todo', async () => {
        ...
        const expectedAction  = {
            type: userActionTypes.LOGIN_USER, user
          }
        ...
        return store.dispatch(userActions.authentication(user.username, user.password)).then(() => {
    ...
    it('should sing_up an action to add a todo', async () => {
        ...
        const expectedAction  = {
          ...
          UserService.create_user.mockImplementationOnce((username, email, password) =>{
              return Promise.resolve(
                user
            )
          })
        ...
        return store.dispatch(userActions.sign_up(user.username, user.email, user.password)).then(() => {
            const action = store.getActions()
            expect(action.length).toBe(1)
            expect(action).toContainEqual(expectedAction)
